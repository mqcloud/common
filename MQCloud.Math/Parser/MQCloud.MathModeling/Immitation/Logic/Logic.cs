...
namespace MQCloud.MathModeling.Immitation.Logic {
    public class Logic {
        ...
        protected void Scedule(string groupName = "", params ITransitionService[] transitionService) {
            transitionService.ForEach(service => Scedule((ITransitionService) service, groupName));
        }
        ...
        protected void Scedule(ITransitionService transitionService, [CallerMemberName] string memberName = "") {
            ...
            Place before = null;
            if (_id - 1 == 0) {
                before = Runtime.Simulation.GetPlace(memberName + ++_id);
                _sceduleAlgorithm.Add(before);
                before.Add(T);
            }

            Place after = Runtime.Simulation.GetPlace(memberName + ++_id);
        ...
        protected void Scedule(Action action, [CallerMemberName] string memberName = "") {
            Transition t = Runtime.Simulation.CreateTransition(memberName + _id);
            t.SetOnReturn(place => {
                action();
                return place.Tokens;
            });
